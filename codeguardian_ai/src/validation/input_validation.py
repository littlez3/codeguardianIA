"""
CodeGuardian AI - Robust Input Validation and Sanitization System
Enterprise-grade input validation using Pydantic with comprehensive sanitization
"""
...
class CodeExecutionRequest(BaseModel):
    ...
    code: constr(min_length=1, max_length=50000) = Field(
        ..., 
        description="Code to execute",
        example="print('Hello World')"
    )
    language: constr(regex=r'^[a-zA-Z][a-zA-Z0-9_]*$', max_length=20) = Field(
        default="python",
...
class ApiKeyCreateRequest(BaseModel):
    ...
    scopes: List[str] = Field(
        default=[],
        description="List of allowed scopes/endpoints"
    )
    expires_days: Optional[conint(ge=1, le=365)] = Field(
        default=30,
